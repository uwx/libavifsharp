...
public static class AvifEncoder
{
    ...
    public static unsafe EncodedImage Encode(byte* pixels, uint width, uint height, uint depth, uint rowBytes, AvifRGBFormat format, Action<EncoderSetttings>? settings = null)
    {
        var encoderSettings = new EncoderSetttings();
        if(settings != null)
        {
        ...
        var image = NativeInterop.avifImageCreate(width, height, depth, encoderSettings.PixelFormat);

        var rgb = new AvifRGBImage();

        NativeInterop.avifRGBImageSetDefaults(in rgb, image);

        rgb.Pixels = (IntPtr)pixels;
